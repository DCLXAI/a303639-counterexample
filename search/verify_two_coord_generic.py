...
def build_problem(kmax: int):
    b_values = [comb(2*k+1, k) for k in range(kmax + 1)]
    shifts = []
    pairs = []
    for c in range(kmax + 1):
        for d in range(c, kmax + 1):
            shifts.append(b_values[c] + b_values[d])
            pairs.append((c, d))
    words = (len(shifts) + 63) // 64
    return shifts, pairs, words


def parse_candidates(path: Path, expected_words: int):
    rows = []
    for line_number, line in enumerate(path.read_text().splitlines(), 1):
        if not line.strip():
            continue
        fields = line.split()
        if len(fields) != 8 + expected_words:
            raise ValueError(
                f"line {line_number}: expected {8+expected_words} fields, got {len(fields)}"
            )
...
def verify_candidate(row, shifts, pairs, timeout_seconds=90):
    obstructed = []
    timeouts = []
    errors = []
    for index in live_indices(row, shifts):
        pair = pairs[index]
...
def main():
    if len(sys.argv) < 4:
        raise SystemExit(
            "usage: verify_two_coord_generic.py K CANDIDATES OUTPUT [workers=2] [timeout=90]"
        )
    kmax = int(sys.argv[1])
    source = Path(sys.argv[2])
    output = Path(sys.argv[3])
    workers = int(sys.argv[4]) if len(sys.argv) > 4 else 2
    timeout_seconds = int(sys.argv[5]) if len(sys.argv) > 5 else 90

    shifts, pairs, words = build_problem(kmax)
    rows = parse_candidates(source, words)
    results = []
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {
            pool.submit(verify_candidate, row, shifts, pairs, timeout_seconds): row
            for row in rows
        }
        for future in as_completed(futures):
            result = future.result()
            results.append(result)
            print(result["status"], result["score1m"], result["n"], flush=True)
